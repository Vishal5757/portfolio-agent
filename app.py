...
def _daily_target_estimate_sell_tax_profile(conn, symbol, sell_qty, sell_price, split_map=None, as_of_date=None, tax_cfg=None, realized_tax_summary=None):
    ...
    # Tax relief: losses can only offset gains that actually exist (IT Act S.70).
    # Pool = prior FY realized gains + this transaction's own net gains.
    # Using NET gains avoids double-counting: tax_payable already nets per bucket,
    # so stcg_net_gain is 0 when losses outweigh gains within the transaction.
    ...
    # Include this transaction's own net gains so cross-bucket intra-transaction
    # offsets work (e.g. STCG net loss vs LTCG gain in the same multi-lot sell).
    total_stcg_available = fy_stcg_realized + stcg_net_gain
    total_ltcg_taxable_available = fy_ltcg_taxable + ltcg_taxable_gain_after_exemption
    # STCG losses: offset STCG gains first, then LTCG taxable gains (IT Act S.70)
    stcg_vs_stcg = min(stcg_net_loss, total_stcg_available)
    stcg_vs_ltcg = min(max(0.0, stcg_net_loss - stcg_vs_stcg), total_ltcg_taxable_available)
    # LTCG losses: only offset remaining LTCG taxable gains
    ltcg_vs_ltcg = min(ltcg_net_loss, max(0.0, total_ltcg_taxable_available - stcg_vs_ltcg))
    tax_relief = round(
...
def _daily_target_trade_match(conn, symbol, side, exec_date, qty, price, exclude_ids=None):
    ...
    for r in rows:
        ...
        # 8% tolerance: wide enough for T+1 slippage on Indian equities where daily
        # moves of 5-7% are common, tight enough to prevent false cross-symbol matches.
        # Recalibration updates ref prices daily, so a fully delayed execution should
        # still land within 8% of the recalibrated price.
        if p > 0 and trade_price > 0 and p_gap / max(p, 1e-9) > 0.08:
            continue
...
def compute_daily_target_performance(conn):
    ...
    closed_pos_rows = conn.execute(
        """
        SELECT entry_value, exit_value, cycle_type
        FROM daily_target_positions
    ...
    for cp in closed_pos_rows:
        ...
        if ev <= 0 or xv <= 0:
            continue
        cycle_type = str(cp["cycle_type"] or "buy_sell").strip().lower()
        if cycle_type == "sell_buyback":
            gross = ev - xv
            sc = _daily_target_zerodha_delivery_costs(ev, exchange="NSE", side="SELL", include_dp_on_sell=True, tax_cfg=_tax_cfg_r)
            bc = _daily_target_zerodha_delivery_costs(xv, exchange="NSE", side="BUY", include_dp_on_sell=False, tax_cfg=_tax_cfg_r)
            taxable = 0.0
        else:
            gross = xv - ev
        ...
        tax = round(max(0.0, net_before_tax if taxable is None else taxable) * _stcg_rate, 2)
    ...
    # Future trade sizing must use take-home realized profit, not the display-only
    # raw realized value. This keeps compounding decisions aligned to charges/tax.
    realized_compounded_capital = round(starting_capital + true_net_realized_profit, 2)
    open_positions = conn.execute(
        """
    ...
    if open_positions:
        ...
        for pos in open_positions:
            ...
            if px > 0 and qty > 0:
                ...
                pos_net_pnl = round(gross_pnl - pos_charges - pos_tax, 2)
                net_live_pnl_est += pos_net_pnl
                latest_balance += pos_net_pnl
            else:
...
def build_daily_target_suggestions(conn, seed_capital=10000.0, target_profit_pct=1.0, top_n=5):
    ...
    used_buy = set()
    used_symbols = set()
    # Running tallies of FY gain pools consumed by earlier pairs in this batch.
    # Each subsequent pair sees a reduced pool so tax-relief alpha is not overstated.
    _running_ltcg_exemption_consumed = 0.0
    _running_stcg_consumed = 0.0       # FY STCG net gains absorbed by prior pairs' STCG losses
    _running_ltcg_taxable_consumed = 0.0  # FY LTCG taxable gains absorbed by prior pairs' losses
    for sell in sell_candidates:
        ...
        for buy in buy_candidates:
            ...
            _batch_adjusted_realized["stcg_net_gain"] = max(0.0, _fy_stcg_gain - _running_stcg_consumed)
            _batch_adjusted_realized["ltcg_net_gain"] = max(0.0, _fy_ltcg_gain - _running_ltcg_taxable_consumed)
            sell_tax = _daily_target_estimate_sell_tax_profile(
                conn,
                sell["symbol"],
                sell_qty,
            ...
            candidate = {
                ...
                # Internal: carry STCG/LTCG net loss for cross-pair pool depletion tracking
                "_sell_stcg_net_loss": round(parse_float(sell_tax.get("stcg_net_loss"), 0.0), 2),
                "_sell_ltcg_net_loss": round(parse_float(sell_tax.get("ltcg_net_loss"), 0.0), 2),
            }
            if best is None or candidate["rotation_score"] > best["rotation_score"]:
                best = candidate
        if not best:
            continue
        # Strip internal tracking fields before storing the pair
        _best_stcg_net_loss = parse_float(best.pop("_sell_stcg_net_loss", 0.0), 0.0)
        _best_ltcg_net_loss = parse_float(best.pop("_sell_ltcg_net_loss", 0.0), 0.0)
        pairs.append(best)
        # Update FY gain pools so subsequent pairs see accurate remaining relief capacity
        _stcg_pool_now = max(0.0, _fy_stcg_gain - _running_stcg_consumed)
        _ltcg_t_pool_now = max(0.0, _fy_ltcg_taxable - _running_ltcg_taxable_consumed)
        _stcg_used = min(_best_stcg_net_loss, _stcg_pool_now)
        _cross_ltcg = min(max(0.0, _best_stcg_net_loss - _stcg_used), _ltcg_t_pool_now)
        _ltcg_used = min(_best_ltcg_net_loss, max(0.0, _ltcg_t_pool_now - _cross_ltcg))
        _running_stcg_consumed += _stcg_used
        _running_ltcg_taxable_consumed += _cross_ltcg + _ltcg_used
...
def _daily_target_llm_prompt(payload):
    summary = dict((payload or {}).get("summary") or {})
    perf = dict((payload or {}).get("performance") or {})
    pairs = []
    for p in list((payload or {}).get("pairs") or [])[:5]:
        pairs.append(
            {
                "rank": p.get("priority_rank"),
                "state": p.get("state"),
                "sell": p.get("sell_symbol"),
                "sell_qty": p.get("sell_qty"),
                "sell_value": p.get("sell_trade_value"),
                "buy": p.get("buy_symbol"),
                "buy_qty": p.get("buy_qty"),
                "buy_value": p.get("buy_trade_value"),
                "buy_target_exit": p.get("buy_target_exit_price"),
                "target_profit": p.get("expected_profit_value"),
                "score": p.get("rotation_score"),
                "progress_pct": p.get("target_progress_pct"),
                "sell_reason": str(p.get("sell_reason") or "")[:500],
                "buy_reason": str(p.get("buy_reason") or "")[:500],
            }
        )
    return (
        ...
        + _safe_json_dumps(
            {
                ...
                "performance": perf,
                "top_pairs": pairs,
                "full_cycle_count": len((payload or {}).get("full_cycles") or []),
            }
        )
    )
...
def attach_daily_target_llm_review(conn, payload):
    ...
    review = {
        ...
        "provider": str(result.get("provider") or ""),
        "model": str(result.get("model") or ""),
        "latency_ms": round(parse_float(result.get("latency_ms"), 0.0), 2),
    ...
    summary["hosted_llm_provider"] = review["provider"]
    summary["hosted_llm_latency_ms"] = review["latency_ms"]
    out["summary"] = summary
    return out


def _recalibrate_daily_target_plan(conn, plan_row, seed_capital=None, target_profit_pct=None, top_n=None, use_hosted_llm=False):
    plan_id = int(parse_float(plan_row.get("id"), 0.0))
    perf = compute_daily_target_performance(conn)
    tax_cfg = get_tax_profile_config(conn)
    realized_tax_summary = compute_realized_equity_tax_summary(conn)
    ...
    if use_hosted_llm:
        payload = attach_daily_target_llm_review(conn, payload)
    return payload


def get_or_create_daily_target_plan(conn, seed_capital=10000.0, target_profit_pct=1.0, top_n=5, recalibrate=True, use_hosted_llm=False):
    active = get_active_daily_target_plan(conn)
    if active:
        if recalibrate:
            return _recalibrate_daily_target_plan(conn, active, seed_capital=seed_capital, target_profit_pct=target_profit_pct, top_n=top_n, use_hosted_llm=use_hosted_llm)
    ...
    def handle_api_get(self, parsed):
        ...
        with db_connect() as conn:
            ...
            if path == "/api/v1/daily-target/plan":
                ...
                llm_raw = qs.get("use_hosted_llm", ["0"])[0]
                try:
                    recalibrate = parse_bool(recalc_raw, default=True)
                    use_hosted_llm = parse_bool(llm_raw, default=False)
                except ValueError:
                    json_response(self, {"error": "recalibrate_or_llm_flag_must_be_boolean"}, HTTPStatus.BAD_REQUEST)
                    return
                try:
