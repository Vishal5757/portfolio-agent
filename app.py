...
def _daily_target_estimate_sell_tax_profile(conn, symbol, sell_qty, sell_price, split_map=None, as_of_date=None, tax_cfg=None, realized_tax_summary=None):
    ...
    # TODO(claude): this still misses same-transaction mixed-bucket offsets.
    # When a sell matches both gain and loss lots, the loss component should first
    # offset the taxable gain generated by that same sell before looking only at
    # previously realized FY gain pools.
...
def _daily_target_trade_match(conn, symbol, side, exec_date, qty, price, exclude_ids=None):
    ...
    for r in rows:
        ...
        # TODO(claude): validate whether this 5% price tolerance is too strict for
        # delayed manual execution flows. It can reject legitimate broker matches
        # when the user executes the planned leg materially away from the original
        # suggested price on a later date.
...
def build_daily_target_suggestions(conn, seed_capital=10000.0, target_profit_pct=1.0, top_n=5):
    ...
    # TODO(claude): later pairs in the same suggestion batch still see the full
    # STCG/LTCG gain pools. Only LTCG exemption is decremented below, so tax-relief
    # alpha can be overstated when top_n > 1 and multiple ideas compete for the
    # same FY gains.
